...
public:
    ForwarderClient(const QHostAddress& address_, quint16 port_)
        : address(address_)
        , port(port_)
    {}
    ForwarderClient(const ForwarderClient* other)
...
class ForwarderConfiguration : public LinkConfiguration
{
    Q_OBJECT
public:
    ...
    ForwarderConfiguration(const QString& name);
    ForwarderConfiguration(ForwarderConfiguration* source);
...
private:
    ...
    QList<ForwarderClient*>   _targetHosts;
    QStringList         _hostList;
    quint16             _localPort;
};

class ForwarderLink : public LinkInterface
{
    Q_OBJECT

public:
    ForwarderLink(SharedLinkConfigurationPtr& config);
    virtual ~ForwarderLink();
...
private:
    ...
    void _writeDataGram     (const QByteArray data, const ForwarderClient* target);

    bool                _running;
    QUdpSocket*         _socket;
    ForwarderConfiguration*   _udpConfig;
    bool                _connectState;
    QList<ForwarderClient*>   _sessionTargets;
