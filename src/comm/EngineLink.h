...
class EngineClient {
public:
    EngineClient(const QHostAddress& address_, quint16 port_)
        : address(address_)
        , port(port_)
    {}
    EngineClient(const EngineClient* other)
...
class EngineConfiguration : public LinkConfiguration
{
    Q_OBJECT
public:
    ...
    EngineConfiguration(EngineConfiguration* source);
    ~EngineConfiguration();
    ...
    const QList<EngineClient*> targetHosts (void)          { return _targetHosts; }

    /// LinkConfiguration overrides
    LinkType    type                 (void) override                                        { return LinkConfiguration::TypeEngine; }
...
private:
    ...
    QList<EngineClient*>   _targetHosts;
    QStringList         _hostList;
    quint16             _localPort;
};

class EngineLink : public LinkInterface
{
    Q_OBJECT

public:
    EngineLink(SharedLinkConfigurationPtr& config);
    virtual ~EngineLink();
...
private:
    ...
    void _writeDataGram     (const QByteArray data, const EngineClient* target);

    bool                _running;
    QUdpSocket*         _socket;
    EngineConfiguration*   _udpConfig;
    bool                _connectState;
    QList<EngineClient*>   _sessionTargets;
