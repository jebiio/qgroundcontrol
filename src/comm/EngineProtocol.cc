...
QGC_LOGGING_CATEGORY(EngineProtocolLog, "EngineProtocolLog")

const char* EngineProtocol::_tempLogFileTemplate   = "FMUDataXXXXXX";   ///< Template for temporary log file
const char* EngineProtocol::_logFileExtension      = "fmu";            ///< Extension for log files

/**
 * The default constructor will create a new MAVLink object sending heartbeats at
 * the MAVLINK_HEARTBEAT_DEFAULT_RATE to all connected links.
 */
EngineProtocol::EngineProtocol(QGCApplication* app, QGCToolbox* toolbox)
    : QGCTool(app, toolbox)
...
void EngineProtocol::setVersion(unsigned version)
{
    /*
...
void EngineProtocol::setToolbox(QGCToolbox *toolbox)
{
   ...
   connect(this, &EngineProtocol::protocolStatusMessage,   _app, &QGCApplication::criticalMessageBoxOnMainThread);
   connect(this, &EngineProtocol::saveTelemetryLog,        _app, &QGCApplication::saveFMULogOnMainThread);
   connect(this, &EngineProtocol::checkTelemetrySavePath,  _app, &QGCApplication::checkFMUSavePathOnMainThread);

   connect(_multiVehicleManager, &MultiVehicleManager::vehicleAdded, this, &EngineProtocol::_vehicleCountChanged);
   connect(_multiVehicleManager, &MultiVehicleManager::vehicleRemoved, this, &EngineProtocol::_vehicleCountChanged);

//    emit versionCheckChanged(m_enable_version_check);
}

void EngineProtocol::loadSettings()
{
...
void EngineProtocol::storeSettings()
{
    // Store settings
    QSettings settings;
    settings.beginGroup("QGC_MAVLINK_PROTOCOL");
...
void EngineProtocol::resetMetadataForLink(LinkInterface *link)
{
}
/*
void EngineProtocol::resetMetadataForLink(LinkInterface *link)
...
void EngineProtocol::logSentBytes(LinkInterface* link, QByteArray b){
 ...
 **/
FmuStream EngineProtocol::parseForwarderPacket(const QByteArray& packet)
{
...
void EngineProtocol::logForwarderPacket(QByteArray packet)
{
...
void EngineProtocol::forwardPacketToEngineServer(const QByteArray& packet)
{
...
bool EngineProtocol::checkValidationPacket(const QByteArray& b)
{
        ...
        if (mavlink_parse_char(mavlinkChannel, static_cast<uint8_t>(b[position]), &_message, &_status)) {
            ...
            if (!link->decodedFirstMavlinkPacket()) {
                ...
                if (!(mavlinkStatus->flags & MAVLINK_STATUS_FLAG_IN_MAVLINK1) && (mavlinkStatus->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1)) {
                    qCDebug(EngineProtocolLog) << "Switching outbound to mavlink 2.0 due to incoming mavlink 2.0 packet:" << mavlinkStatus << mavlinkChannel << mavlinkStatus->flags;
                    mavlinkStatus->flags &= ~MAVLINK_STATUS_FLAG_OUT_MAVLINK1;
                    // Set all links to v2
                    setVersion(200);
                }
 ...
 **/
QString EngineProtocol::getName()
{
    return tr("MAVLink protocol");
}

/** @return System id of this application */
int EngineProtocol::getSystemId() const
{
    return systemId;
}

void EngineProtocol::setSystemId(int id)
{
    systemId = id;
}

/** @return Component id of this application */
int EngineProtocol::getComponentId()
{
    return MAV_COMP_ID_MISSIONPLANNER;
}

void EngineProtocol::enableVersionCheck(bool enabled)
{
    // m_enable_version_check = enabled;
    emit versionCheckChanged(enabled);
}

void EngineProtocol::_vehicleCountChanged(void)
{
...
bool EngineProtocol::_closeLogFile(void)
{
...
void EngineProtocol::_startLogging(void)
{
    ...
    if (!_tempLogFile.isOpen()) {
        if (!_logSuspendReplay) {
            ...
            qCDebug(EngineProtocolLog) << "Temp log" << _tempLogFile.fileName();
...
void EngineProtocol::_stopLogging(void)
{
    if (_tempLogFile.isOpen()) {
        if (_closeLogFile()) {
...
void EngineProtocol::checkForLostLogFiles(void)
{
...
void EngineProtocol::suspendLogForReplay(bool suspend)
{
    _logSuspendReplay = suspend;
}

void EngineProtocol::deleteTempLogFiles(void)
{
    QDir tempDir(QStandardPaths::writableLocation(QStandardPaths::TempLocation));
